...
<title>需求分析管理</title>
    ...
    <script type="text/javascript">
        $(function(){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/ana/showAllAnaInfo',
                dataType:'json',
                success:function(rs){
                    $(rs).each(function(index,item){
                        var date1 = new Date(item.addtime );
                        var addtime = date1.getFullYear() + "-" + (date1.getMonth() + 1) + "-" + date1.getDate();
                        var date2 = new Date(item.updatetime);
                        ...
                        var tr ="<tr align='center' bgcolor=\"#FFFFFF\" onMouseMove=\"javascript:this.bgColor='#FCFDEE';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\" height=\"22\" >\n" +
                            ...
                            "\t<td>"+item.id+"</td>\n" +
                            "\t<td align=\"left\"><a href=''><u>"+item.title+"</u></a></td>\n" +
                            "\t<td>"+item.project.pname+"</td>\n" +
                            "\t<td>"+addtime+"</td>\n" +
                            "\t<td>"+updatetime+"</td>\n" +
