...
<title>功能管理</title>
    ...
    <script type="text/javascript">
        $(function(){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/fun/showAllFunInfo',
                dataType:'json',
                success:function(rs){
                    $(rs).each(function(index,item){
                        var date1 = new Date(item.module.analysis.addtime );
                        var addtime = date1.getFullYear() + "-" + (date1.getMonth() + 1) + "-" + date1.getDate();
                        var date2 = new Date(item.module.analysis.updatetime);
                        ...
                        var tr ="<tr align='center' bgcolor=\"#FFFFFF\" onMouseMove=\"javascript:this.bgColor='#FCFDEE';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\" height=\"22\" >\n" +
                            ...
                            "\t<td>"+item.id+"</td>\n" +
                            "\t<td align=\"left\"><a href=''><u>"+item.functionname+"</u></a></td>\n" +
                            "\t<td>"+item.module.modname+"</td>\n" +
                            "\t<td>"+item.module.analysis.title+"</td>\n" +
                            "\t<td>"+item.module.analysis.project.pname+"</td>\n" +
                            "\t<td>"+item.level+"</td>\n" +
                            "\t<td>"+addtime+"</td>\n" +
                            "\t<td>"+updatetime+"</td>\n" +
