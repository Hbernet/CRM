    ...
    <script type="application/javascript">
        $(function(){
            $.ajax({
                type:'GET',
                //获取需求有模块的项目
                url:'${pageContext.request.contextPath}/ana/showModInfo',
                dataType:'json',
                success:function(resultData){
                    $(resultData).each(function(index,item){
                        var option="<option value="+item.id+","+item.title+">"+item.project.pname+"</option>";
                        $("#pro").append(option);
                    });
                },
            });
        ...
        function addayalisys(obj){
            // 给需求分析添加值
            $("#anid").val(obj.split(",")[1]);
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/mod/getModInfo',
                data:{"id":obj.split(",")[0]},
                dataType:'json',
                success:function(resultData){
                    $("#mod option").remove();
...
<tr >
	...
	<td  align='left' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
		<input id="anid" disabled="disabled" type="text" name="analysisname">
	</td>
</tr>
<tr >
